...
def Local(edge_list, x_0, u_0, entity_labels=None):
    ...
    r = scatter(
        u[index_2].type(torch.DoubleTensor).to(u.device), index_1, dim_size=len(x)
    )  # count the known neighbors
    prim_nodes = (r > 0) & (
        u == 0
    )  # indicates if an unknown node has neighbor with a known value
    ...
    if (u == 0).any():  # check isolated components
        if entity_labels is None:
            ...
            for label in np.unique(entity_labels[u.cpu().data.numpy() == 0]):
                avg = (
                    x[((u == 1) & (entity_labels == label)).to(torch.bool)]
                    .mean()
                    .item()
                )  # average of the propagated value in those entities
                if not np.isnan(avg):
                    # Create the mask and apply the average
                    mask = ((u == 0) & (entity_labels == label)).to(
                        torch.bool
                    )  # Ensure the mask is a boolean tensor
                    x[mask] = avg
                else:
                    # If no valid average, apply the global mean for entities where u == 1
                    mask = ((u == 0) & (entity_labels == label)).to(
                        torch.bool
                    )  # Ensure the mask is a boolean tensor
                    x[mask] = (x[u == 1]).mean().item()
    return x
